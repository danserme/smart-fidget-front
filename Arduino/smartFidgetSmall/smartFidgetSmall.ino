...
int minHB = 0, maxHB = 0, avgHB = 0, beats = 0, times = 0;
...
void writeFile() {
  ...
  newData["max"] = maxHB;
  newData["avg"] = avgHB;
  newData["p"] = String(counterP);
  newData["ccw"] = String(counterCCW);
  newData["cw"] = String(counterCW);
  ...
  // flash.eraseSector(address);
  if (flash.writeBuffer(currentAddress, buffer, bytesWritten)) {
